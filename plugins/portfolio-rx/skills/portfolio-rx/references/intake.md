...
- Homepage is a JS-only SPA with no server-rendered content. WebFetch will return shell HTML. Say: "This site is rendered client-side — WebFetch only sees the shell. Share screenshots of the case studies, or paste the copy."
...
- Site is paywalled / behind Cloudflare challenge / requires JS to clear. Same script as above: name what's wrong, name the move.
